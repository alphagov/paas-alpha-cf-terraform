...
resource "template_file" "cf_stub" {
    ...
    vars {
        ...
        region                  = "${var.region}"
        public_z1_subnet_id     = "${aws_subnet.public.0.id}"
        public_z2_subnet_id     = "${aws_subnet.public.1.id}"
        private_z1_subnet_id    = "${aws_subnet.private.0.id}"
        private_z2_subnet_id    = "${aws_subnet.private.1.id}"
        aws_secret_access_key   = "${var.AWS_SECRET_ACCESS_KEY}"
        aws_access_key_id       = "${var.AWS_ACCESS_KEY_ID}"
